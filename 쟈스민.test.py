...
if __name__ == "__main__":
    ...
    model = deeplabv3plus_mobilenet(num_classes=15, output_stride=16)
    model.load_state_dict(torch.load('./mobilenet_30000_model.pth')['model_state'])
    ...
    with torch.no_grad():
        for i, images in tqdm(enumerate(test_loader)):
            ...
            pred = test_loader.dataset.decode_target(pred).astype(np.uint8)[:, :, 0]

            Image.fromarray(pred).save(f'{save_path}/{i}_image.png')